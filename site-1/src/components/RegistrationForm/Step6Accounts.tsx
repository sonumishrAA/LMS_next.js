...
export default function Step6Accounts({
  ...
  const checkEmailExists = async (email: string) => {
    ...
    try {
      const respData = await callEdgeFunction('check-email', {
        body: { email }
      })
  ...
  const handleVerify = async () => {
    ...
    try {
      const respData = await callEdgeFunction('verify-owner-password', {
        body: { email: emailValue, password: verifyPassword }
      })
